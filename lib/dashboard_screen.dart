...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // --- منطق المزامنة الجديد (إضافة) ---
  int userPoints = 50; // هذه القيمة هي التي ستتم مزامنتها مع واجهة العروض
  ...
  void _calculateAmount(String value) {
    ...
    setState(() {
      ...
      if (amount < 2000) { 
        ...
        // --- تعديل منطق العمولة للمزامنة مع النقاط ---
        if (userPoints >= 50) {
          _commission = 0; // إعفاء المستخدم عند وصوله لـ 50 نقطة
                  ...
                  if (_amountController.text.isNotEmpty && !_isInvalidAmount)
                    Padding(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween, 
                        children: [
                          // تعديل لون العمولة للأخضر في حال الإعفاء
  ...
  void _showDoubleCheckDialog() {
    showDialog(context: context, builder: (context) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), title: const Text('تأكيد نهائي'), content: const Text('هل أنت متأكد من إرسال الطلب؟'), actions: [
      TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')), 
      ElevatedButton(onPressed: () { 
        // --- منطق تصفير النقاط عند التأكيد النهائي والنجاح ---
        if (userPoints >= 50) {
          setState(() { userPoints = 0; });
        }
